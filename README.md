...
├── requirements.txt
├── config/
│   ├── baseline.yaml          # Default RAG configuration
│   └── search_space.yaml      # All tunable parameters and ranges
├── data/
│   ├── golden_qa.json         # 50 expert-verified Q&A pairs
│   ├── synthetic/             # RAGAS-generated test cases
│   └── adversarial/           # Edge cases for robustness testing
├── src/
│   ├── agent/
│   │   ├── controller.py      # Main agent loop (hypothesize-execute-evaluate-decide)
│   │   └── strategy.py        # MAB exploration strategy
│   ├── retriever/
│   │   ├── hybrid_search.py   # ChromaDB + BM25 fusion
│   │   ├── reranker.py        # Cross-encoder reranking
│   │   └── chunker.py         # Configurable chunking strategies
│   ├── generator/
│   │   └── llm.py             # LLM wrapper (Mistral/Llama)
│   ├── evaluation/
│   │   ├── ragas_eval.py      # RAGAS metric computation
│   │   ├── ir_metrics.py      # NDCG, MRR, Recall@k, Precision@k
│   │   └── statistics.py      # Bootstrap CIs, effect sizes, significance tests
│   └── utils/
...
│   ├── pages/
│   │   ├── config_lab.py      # Configuration & run experiments
│   │   ├── results.py         # Metric dashboards
│   │   ├── comparison.py      # Side-by-side comparison
│   │   ├── trajectory.py      # Agent decision visualization
│   │   └── export.py          # Export results
│   └── components/
│       └── charts.py          # Plotly chart builders
├── experiments/
│   └── logs/                  # Agent decision logs (JSON)
├── notebooks/
...
```bash
git clone https://github.com/stanislav-li/rag-optimization.git
cd rag-optimization
...
```bash
python src/agent/controller.py --config config/baseline.yaml
```
